...
class Loging():
    ...
    def inicioSesion():
        if request.method == 'POST':
            ...
            if user:
                ...
                if  tipo == True:
                    return render_template('adminCitas.html')
                else:
    ...
    def citas():
        if request.method == 'POST':
            ...
            user = session.query(Usuario.id).filter_by(correo='carlos@gmail.com',contraseña=1234).scalar()
            ...
            msg = Message('Hello', sender = 'a170069@unach.mx', recipients = [correo])
    ...
    @loginRoute.route('/desconexion',methods=['GET'])
    def logout():
         return render_template('login.html')
